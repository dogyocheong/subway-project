...
// indoor-nav-data.json is generated by scripts/analyze-station-maps.ts
// It lives in src/routes/ and is updated live — resolve from process.cwd() (project root)
// so the path stays correct whether the server runs from src (tsx) or dist (node).
function loadIndoorNavData(): any[] {
...
// GET /subway/indoor-nav/:stationName?line=
router.get("/subway/indoor-nav/:stationName", (req, res) => {
  const stationName = decodeURIComponent(req.params.stationName || "");
  const lineFilter = req.query.line as string | undefined;

  if (!stationName) { res.status(400).json({ error: "역명이 필요합니다" }); return; }

  const navData = loadIndoorNavData();

  if (navData.length === 0) {
    res.status(404).json({ error: "역 안내 데이터를 아직 처리 중입니다. 잠시 후 다시 시도해 주세요." });
    return;
  }

  // Search: exact name match first, then partial
  let matches = navData.filter((item: any) =>
    item.stationName === stationName ||
    item.stationName?.includes(stationName) ||
    stationName.includes(item.stationName)
  );

  if (matches.length === 0) {
    res.status(404).json({ error: `'${stationName}' 역 안내 데이터가 없습니다.` });
    return;
  }

  // If line filter specified, narrow down
  if (lineFilter && matches.length > 1) {
    const lineMatches = matches.filter((item: any) =>
      item.line?.includes(lineFilter) || item.line === `${lineFilter}호선`
    );
    if (lineMatches.length > 0) matches = lineMatches;
  }

  const primary = matches[0];
  // Include all matches as allResults for transfer stations
  const allLines = matches.length > 1
    ? matches.map((m: any) => ({ line: m.line, voiceGuide: m.voiceGuide }))
    : undefined;
  res.json({ ...primary, allLines });
});
